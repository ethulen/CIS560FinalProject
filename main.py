        ...
        def search_customers(customer_search):
            if customer_search != "":
                display(
                    "SELECT CustomerID, CustomerName, Street, City, State FROM "
                    "FoundationElectronics.Customer C"
                    "ORDER BY CustomerName ASC", customer_search
                )
...
class FindEmployeeMenu(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def display(search, criteria1):
            ...
            if search == "":
                my_cursor = cursor.execute(
                    "SELECT EmployeeID, EmployeeName, StartDate, S.StoreID FROM "
                    "FoundationElectronics.Employee E FULL JOIN FoundationElectronics.Store S ON S.StoreID = "
                    "E.StoreID ORDER BY EmployeeName ASC"
                )
            else:
                my_cursor = cursor.execute(search, criteria1)
        ...
        def delete_employee(employee_id):
            cursor.execute("UPDATE FoundationElectronics.Purchase SET IsDeleted = 1 WHERE PurchaseID = ?", employee_id)
        ...
        def search_employees(employee_search):
            if employee_search != "":
                display(
                    "SELECT EmployeeID, EmployeeName, StartDate, S.StoreID FROM "
                    "FoundationElectronics.Employee E FULL JOIN FoundationElectronics.Store S ON S.StoreID = "
                    "E.StoreID ORDER BY EmployeeName ASC", employee_search
                )
...
class FindProductMenu(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(self, text="Refresh", command=lambda: display("", ""))
        button2 = tk.Button(self, text="Back", command=lambda: [controller.show_frame("ProductFunctionsMenu")])
        entry1 = tk.Entry(self, bg="white")
        ...
        def display(search, criteria1):
            ...
            for purchase in my_cursor:
                for j in range(len(purchase)):
                    ...
                    e.grid(row=i, column=j + 1)
                e = tk.Button(scroll_data, width=5, text='Edit', relief='ridge',
                              anchor="w", command=lambda k=purchase[0]: controller.show_frame("UpdateProductMenu"))
                e.grid(row=i, column=6)
                f = tk.Button(scroll_data, width=5, text='Delete', relief='ridge',
        ...
        def delete_product(product_id):
            cursor.execute("UPDATE FoundationElectronics.Purchase SET IsDeleted = 1 WHERE PurchaseID = ?", product_id)
        ...
        def search_products(product_search):
            if product_search != "":
                display(
                    "SELECT ProductName, ItemCost, S.SupplierID FROM "
                    "FoundationElectronics.Product P FULL JOIN FoundationElectronics.Supplier S ON S.SupplierID = "
                    "P.SupplierID ORDER BY ProductName ASC", product_search
                )
